...
void checkCufft(cufftResult result)
{
	if(result!=CUFFT_SUCCESS){
		printf(" cuFFT did not successfully created the FFT plan, the reason is:\n");
...
extern void initialize_cufft(GPU_INFO *gpu_info,CUFFT_INFO *cufft_info){
	...
	whichGPUs=(int*)malloc(sizeof(int)*(gpu_info->GPU_N));
	
	for(i=0;i<(gpu_info->GPU_N);i++)
		whichGPUs[i]=i;	//!Define on these GPU to calculate 
	...
	worksize=(size_t *)malloc(sizeof(size_t)*(gpu_info->GPU_N));
	...
	checkCufft(cufftMakePlanMany (cufft_info->plan_forward, rank, Dim, NULL, 1, 1, NULL, 1, 1, CUFFT_D2Z, batch, worksize));
	checkCufft(cufftMakePlanMany (cufft_info->plan_backward, rank, Dim, NULL, 1, 1, NULL, 1, 1, CUFFT_Z2D, batch, worksize));

	checkCufft(cufftXtMalloc (cufft_info->plan_forward, &cufft_info->device_in, CUFFT_XT_FORMAT_INPLACE)); 
	checkCufft(cufftXtMalloc (cufft_info->plan_backward, &cufft_info->device_out, CUFFT_XT_FORMAT_INPLACE)); 
	...
	dp=fopen("phi.dat","r");
	fgets(comment,200,dp);
	fgets(comment,200,dp);
	for(ijk=0;ijk<NxNyNz;ijk++){
		fscanf(dp,"%lg %lg %lg %lg\n",&temp,&temp,&cufft_info->wa[ijk],&cufft_info->wb[ijk]);
...
extern void D1_MultipleGPU(GPU_INFO *gpu_info,data_assem *data_test,int N){
	...
	size_t worksize[2]; cufftComplex *host_data_input, *host_data_output; 
	...
	if(result!=CUFFT_SUCCESS){
		printf("make plan GPU failed\n");
		if(result==CUFFT_INVALID_PLAN) printf("he plan parameter is not a valid handle.");
		else if(result==CUFFT_ALLOC_FAILED) printf("allocates resource failed\n");
		else if(result==CUFFT_INVALID_VALUE)printf("invalid parameters\n");
		else if(result==CUFFT_INTERNAL_ERROR)printf("internal errors\n");
		else if(result==CUFFT_SETUP_FAILED) printf("CUFFT_SETUP_FAILED\n");
		else if(result==CUFFT_INVALID_SIZE) printf("CUFFT_INVALID_SIZE , nx is too small\n");
	}
